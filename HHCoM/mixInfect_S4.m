...
prepPauseYear = 2025 + (1/6); % Year where PrEP use temporarily stops
restartYear = 2025 + (3/6); % Year when PrEP usage starts increasing again
finalPeakYear = 2030.5; % Year when PrEP reaches its peak again
...
elseif year >= prepPauseYear && year < restartYear %PEPFAR Stopage
    prepUse = 0.0168;

elseif year >= restartYear && year <= finalPeakYear
    % After the pause, reintroduce PrEP with a linear increase reaching
    % peak in 2030.5
            ...
            for d = 1 : disease
                for p = 1 : intervens
                    ...
                    % This is the time period within the first effPeriod when everyone has full vax efficacy
                        ...
                        vaxProtect_NORM = lambdaMultVax(2 , 1); % you know you are within effPeriod, so everyone who is vaccinated would have full efficacy
