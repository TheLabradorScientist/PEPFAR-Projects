...
for n = 1 : nSims
   for gInd = 1 : 3
        ...
        for i = 1 % : length(inds)
            ...
            hivIncRef = ...
                (annlz(sum(sum(sum(vaxResult{n}.newHiv(: , g , 4:10 , :),2),3),4)) ./ ...
...
for n = 1 : nSims
   for gInd = 1 : 3
        ...
        for i = 1 : length(inds)
            ...
            hpvIncRef = ...
                ((annlz(sum(sum(sum(sum(vaxResult{n}.newHpv(: , g , inds{i} , 4:age , :),2),3),4),5))+annlz(sum(sum(sum(sum(vaxResult{n}.newImmHpv(: , g , inds{i} , 4:age , :),2),3),4),5))+annlz(sum(sum(sum(sum(vaxResult{n}.newVaxHpv(: , g , inds{i} , 4:age , :),2),3),4),5))) ./ ...
                (annlz(sum(vaxResult{n}.popVec(: , genArray{i}) , 2) ./ stepsPerYear)) * fac);
...
for n = 1 : nSims
   for gInd = 1
        ...
        for i = 1 : length(inds)
            ...
            hpvIncRef_nVax = ...
                ((annlz(sum(sum(sum(sum(vaxResult{n}.newHpv(: , g , : , 4:age , :),2),3),4),5))+annlz(sum(sum(sum(sum(vaxResult{n}.newImmHpv(: , g , : , 4:age , :),2),3),4),5))) ./ ...
