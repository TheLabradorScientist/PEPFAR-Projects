...
for g = 1 : gender
    for i = 1 : length(inds)
        ...
        noV_Hpv = ...
            annlz(sum(sum(sum(sum(noV.newHpv(: , g , inds{i} , 4 : age , :),2),3),4),5)) ./ ...
            (annlz(sum(noV.popVec(: , genArray{i}) , 2) ./ stepsPerYear))* fac;

        c70_2vPartial_Inc = ...
            annlz(sum(sum(sum(sum(c70_2vPartial.newHpv(: , g , inds{i} , 4 : age , :),2),3),4),5)) ./ ...
            (annlz(sum(c70_2vPartial.popVec(: , genArray{i}) , 2)) ./ stepsPerYear) * fac;

        c90_9vFullInc = ...
            annlz(sum(sum(sum(sum(c90_9vFull.newHpv(: , g , inds{i} , 4 : age , :),2),3),4),5)) ./ ...
            (annlz(sum(c90_9vFull.popVec(: , genArray{i}) , 2) ./ stepsPerYear)) * fac;

        c90_2vFullInc = ...
            annlz(sum(sum(sum(sum(c90_2vFull.newHpv(: , g , inds{i} , 4 : age , :),2),3),4),5)) ./ ...
...
popTot = noV.popVec(: , toInd(allcomb(1 : disease , 1 : viral , 1 : hpvTypes , ...
    1 : hpvStates , 1 : periods , 1 : 2 , 4 : age, 1 : risk)));
