...
%     hold on
    subplot(2,1,1);
    plot(1 : length(hpvHIV2017) , hpvHIV2017 , 'o-' , 'Color' , linColor{j});
    hold all;
    subplot(2,1,2);
    plot(1 : length(hpvNeg2017) , hpvNeg2017 , 'o-' , 'Color' , linColor{j})
    hold all;
    ...
    errorbar(1 : length(hpvHivObs) , hpvHivObs(: , 1) , yNegError , yPosError , 'rs')
    set(gca , 'xtick' , 1 : length(hpvHIV2017) , 'xtickLabel' , ageGroup);
    xlabel('Age Group'); ylabel('hrHPV Prevalence in 2002 (%)')
    title('Among HIV-positive')
    hold all;
    %HIV-
    subplot(2,1,2);
