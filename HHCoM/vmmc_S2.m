...
elseif year >= 2014 && year < 2025
    ...
    periodInd = 4;
elseif year >= 2025 && year < (2025 + 1/6) %PEPFAR stoppage
    ind = (round(vmmcYr_vec{5}, 4) == round(year, 4));
    periodInd = 5;
elseif year >= (2025 + 1/6) && year < (2025 + 2/6) %PEPFAR Stoppage
    ind = (round(vmmcYr_vec{6}, 4) == round(year, 4));
    periodInd = 6;
elseif year >= (2025 + 2/6) && year < 2030.5
    ind = (round(vmmcYr_vec{7}, 4) == round(year, 4));
    periodInd = 7;
elseif year >= 2030.5
    ind = length(vmmcYr_vec{7});
    periodInd = 7;
end
