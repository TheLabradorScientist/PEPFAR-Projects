function [vax, deaths, ccHealthState, hpvHealthState, newCC, totalPerAge, screenTreat, screenSympCCTreat, hivHealthState, newHiv] = vaxCEA_multSims_processResults_HIV(vaxResultInd , sceNum , fileNameNums, fileInds, vax, deaths, ccHealthState, hpvHealthState, newCC, totalPerAge, screenTreat, screenSympCCTreat, hivHealthState, newHiv)
...
% CC HEALTH STATES (CC stage prevalence) ****************************
    ...
    for a = 1 : age 
        for dInd = 1 : length(diseaseVec_vax)
            d = diseaseVec_vax{dInd}; 
	       for x = 1 : endpoints 
    		    vaxInds1 = toInd(allcomb(d, 1:viral, 1:hpvVaxStates, 6, x, 1:intervens, 2, a, 1:risk));
                vaxInds2 = toInd(allcomb(d, 1:viral, 6, [1:5 7], x, 1:intervens, 2, a, 1:risk)); 
    		    ...
    		    ccHealthState(1:end, dInd, a, x, j) = sum(vaxResult{n}.popVec(:, vaxInds), 2);
            end 
	    end 
    end 
