...
toNowName = ['toNow_15Jan_stochMod_19']
% %Final version 
% toNowName = ['toNow_determMod_final']
...
sheet = ['pop'];
cols1 = {toNowName};
cols2 = {'NoArtDiscontFix'}; %, 'UN pop'};
xlswrite(filename, cols1, sheet, 'D1')
xlswrite(filename, cols2, sheet, 'D2')
xlswrite(filename, [totalPopVec(1 : stepsPerYear * 5 : end)'], sheet, 'D3')
...
sheet = ['HIV_prev'];
cols1 = {toNowName};
cols2 = {'NoArtDiscontFix'} %, 'ANC data (Kisumu)', 'Year', 'Spectrum data (Nyanza)'};
xlswrite(filename, cols1, sheet, 'U1')
xlswrite(filename, cols2, sheet, 'U2')
xlswrite(filename, [hivPrev(331:stepsPerYear:end) ], sheet, 'U3')
% xlswrite(filename, [HIV_ANC_Kisumu'], sheet, 'C3')
% xlswrite(filename, [HIV_Kenya_spectrum'], sheet, 'E3')

%% HIV mortality over time by sex vs. observed 
kisumu_hivMortF = [2008 69.4 0.8; 2011 17.3 3.5]; %observed mortality rates reported by Reniers, et al  
kisumu_hivMortM = [2008 183.3 5; 2011 42.4 5.2];

figure()
for g = 1 : 2
        %a = ageVec{aInd};
        popAge = toInd(allcomb(3 : disease , 1 : viral , 1 : hpvVaxStates , 1 : hpvNonVaxStates , ...
            1 : endpoints , 1 : intervens , g , 4:13 , 1 : risk));
        popTot = toInd(allcomb(1 : 2 , 1 : viral , 1 : hpvVaxStates , 1 : hpvNonVaxStates , ...
            1 : endpoints , 1 : intervens , g  , 4 : 13 , 1 : risk));
        bkgdDeaths(:, g) = annlz(sum(deaths(1:end-1, popTot), 2)) .* 1000;  
        popTotAge(:, g) = annlz(sum(popVec(1:end-1 , popTot), 2)) ./ stepsPerYear;
        nonHivdeaths(:, g) = bkgdDeaths(:, g) ./ popTotAge(:, g);
        
        hivDeathsNum(:, g) = annlz(sum(hivDeaths(1:end-1, :, g, 4:13), 3)) .* 1000 ;
        popTotAgeAll(:, g) = annlz(sum(popVec(1:end-1 , popAge), 2)) ./ stepsPerYear;
        hivDeathsRate(:, g) =  hivDeathsNum(:, g) ./ popTotAgeAll(:, g);
        
        plot(tVec(1:stepsPerYear:end-1), hivDeathsRate(:, g), tVec(1:stepsPerYear:end-1), nonHivdeaths(:, g))
        hold all
end

plot(kisumu_hivMortM(: ,1 ), kisumu_hivMortM(:, 2), 'o', ...
    kisumu_hivMortM(: ,1 ), kisumu_hivMortM(:, 3), 'o',...
    kisumu_hivMortF(:, 1), kisumu_hivMortF(: ,2), '+', ...
    kisumu_hivMortF(: ,1 ), kisumu_hivMortF(:, 3), '+')
xlim([1980 2020])
ylabel('Mortality rates (per 1000)')
legend('Model, HIV+ male', 'Model, HIV- male', 'Model, HIV+ female', 'Model, HIV- female',...
    'Kisumu, HIV+ male', 'Kisumu, HIV- male', 'Kisumu, HIV+ female', 'Kisumu, HIV- female', 'Location', 'NorthEast')
title('Mortality rates by HIV status')
...
%% HIV prevalance by sex, all ages 
figure()

file = [pwd , '/Config/Kenya_parameters_Feb20.xlsx'];
HIV_spectrum_sex(:, 3) = xlsread(file , ['HIV ' 'prevalence'] , 'B184:B213'); %year
HIV_spectrum_sex(:, 2) = xlsread(file , ['HIV ' 'prevalence'] , 'I184:I213'); %female
HIV_spectrum_sex(:, 1) = xlsread(file , ['HIV ' 'prevalence'] , 'L184:L213'); %male
...
legend('Males, model' , 'Males, DHS/KAIS', 'Male, UNAIDS', ...
    'Females, model', 'Females, DHS/KAIS', 'Female, UNAIDS',...
    'Location', 'Northwest')

%xlswrite(filename, [hivObsGender(:, 3), hivObsGender(:, 1:2)], sheet, 'E3')

%% HIV prevalence by risk group, all ages 
figure()
...
xlabel('Year')
xlim([1980 2020])
title('HIV Prevalence by age over time')
legend(ageGroup, 'Location', 'Northwest')
...
legend('15-19, model' , '20-24, model ' , '25-29, model' ,...
    ...
    'Location' , 'NorthWest');
 %%
 fname = 'U:\Kenya_model_HPV-HIVacq\HHCoM\\HHCoM_Results\histo_circ.xlsx';
 circtable = array2table([tVecYr(56:96)' circProp(331 : stepsPerYear :end, :)]); 
 circtable.Properties.VariableNames(1:size(circtable, 2)) = {'Year', '15-19','20-24', '25-29', '30-39','40-49', '50+'};
 writetable(circtable, fname);
...
%% HPV prevalence by risk group in women 
riskgrps = {'Low risk' , 'Medium risk', 'High risk'};
figure()
for r = 1 : risk
    % General
    hpvInds = unique([toInd(allcomb(1 : disease , 1 : viral , 2 : 5 , [1 : 5 , 7] , ...
        1 , 1 : intervens , 2 , 4 : age , r)); toInd(allcomb(1 : disease , 1 : viral , ...
        [1 : 5 , 7] , 2 : 5 , 1 , 1 : intervens , 2 , 4 : age , r))]);
    hpvPop = sum(popVec(: , hpvInds) , 2);
    popTot = popVec(: , toInd(allcomb(1 : disease , 1 : viral , 1 : hpvVaxStates , 1 : hpvNonVaxStates , ...
        1 : endpoints , 1 : intervens , 2 , 4 : age , r)));
    genPopHPV = 100 * hpvPop ./ sum(popTot , 2);
    % HIV+
    hpvHivInds = unique([toInd(allcomb(3 : 7 , 1 : viral , 2 : 5 , [1 : 5 , 7] , ...
        1 , 1 : intervens , 2 , 4 : age , r)); toInd(allcomb(3 : 7 , 1 : viral , ...
        [1 : 5 , 7] , 2 : 5 , 1 , 1 : intervens , 2 , 4 : age , r))]);
    hpvHivPop = sum(popVec(: , hpvHivInds) , 2);
    popHivTot = popVec(: , toInd(allcomb(3 : 7 , 1 : viral , 1 : hpvVaxStates , 1 : hpvNonVaxStates , ...
        1 : endpoints , 1 : intervens , 2 , 4 : age , r)));
    ...
    %ART
    hpvArtInds = unique([toInd(allcomb(8 , 6 , 2 : 5 , [1 : 5 , 7] , ...
        1 , 1 : intervens , 2 , 4 : age , r)); toInd(allcomb(8 , 6 , ...
        [1 : 5 , 7] , 2 : 5 , 1 , 1 : intervens , 2 , 4 : age , r))]);
    hpvArtPop = sum(popVec(: , hpvArtInds) , 2);
    popArtTot = popVec(: , toInd(allcomb(8 , 6 , 1 : hpvVaxStates , 1 : hpvNonVaxStates , ...
        1 : endpoints , 1 : intervens , 2 , 4 : age , r)));
    ...
    plot(tVec , artHPV)
     xlim([1950 2020])
    ylim([0 90])
    xlabel('Year'); ylabel('Prevalence (%)'); title(['HPV Prevalence in ', riskgrps{r} ,' (ages 15+)'])
    legend('General' , 'HIV-' , 'HIV+' , 'ART' , 'Location' , 'NorthWest')
end
...
yr = 2015;
year = {yr};
cinHiv_ccScreen = [0.18	0.105 0.089] .* 100; % Observed, HIV+, Nairobi, Memiah, 2012; data collected 2009-2010
cinHiv_ccScreen2 = [33.1 10.5 12.8]; %Observed, HIV clinic, De Vuyst 2013, data collected 2009
cinHIV_FSW = [19.1 NaN 13.1]; %Observed, HIV+ FSW, Nairobi, Sweet 2020, data collected 2009-2011
cinNeg_FSW = [8.0 NaN 1.5]; % Observed, HIV- FSW, Nairobi, Sweet 2020
cinHIV_FSW2 = [17.9 NaN 11.9]; %Observed, HIV+ FSW, Nairobi, Njagi 2013
cinNeg_FSW2 = [9.0 NaN 1.88]; %Observed, HIV- FSW, Nairobi, Njagi 2013, data collected 2009-2012
...
% xlswrite(filename, plotTits1, sheet, 'F2')
...
%% HPV incidence by risk group and HIV status 
figure()
for r = 1 : risk
    hpvSusInds = [toInd(allcomb(1 : 2 , 1 : viral , 1 : hpvVaxStates , 1 , ...
        1 : endpoints , 1 : intervens, 2 , 4:16 , r)); ...
