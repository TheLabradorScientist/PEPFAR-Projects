...
pathModifier = ['toNow_' , date , '_stochMod_' , 'baseline_2dose_nowanePrep' , num2str(paramSetIdx)]; % ***SET ME***: name for historical run output file 
...
% SCREENING %SCREEN ALG NEEDS TO BE 2 FOR CISNET
...
if ~ isfile([pwd , 'HHCoM_Results/' , pathModifier , '.mat'])
    ...
    prepCov = zeros(length(s) - 1 , gender, age);
    condCov = zeros (length(s) - 1 , gender, disease , age, risk);
...
elseif isfile([pwd , 'HHCoM_Results/' , pathModifier , '.mat'])
    ...
    prepCov = chckPntIn.prepCov;
    condCov = chckPntIn.condCov;
...
%         end 
    ...
    if rem(year , 25) == 0.0
        ...
        save(fullfile(savdir , pathModifier, '') , 'fivYrAgeGrpsOn' , 'tVec' ,  'popVec' , 'newHiv' , 'prepCov' , ...
            'condCov' ,'newHpvVax' , 'newImmHpvVax' , 'newHpvNonVax' , 'newImmHpvNonVax' , ...
