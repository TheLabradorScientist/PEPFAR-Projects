...
function vaxCEA_multSims_mainFunction_PEPFAR_S1(username)
...
%for k = 1 : loopSegmentsLength-1 
 ...
 for j = [1] % FORTESTING
    ...
    hivDeaths = zeros(nTimepoints, gender, age+1, nRuns);     ccHealthState = zeros(nTimepoints, length(diseaseVec_vax), age+1, endpoints, nRuns); 
    ...
    % Feeding in the zeroed result matrix, spitting out the same matrix but with all the counts added in for that scenario
     [vax, ccDeaths, hivDeaths, ccHealthState, hpvHealthState, newCC, totalPerAge, screenTreat, screenSympCCTreat, hivHealthState, newHiv, prepCov, newCirc] = ...
        vaxCEA_multSims_processResults_PEPFAR_S1(1 , sceString , {'0'}, fileInds, vax, ccDeaths, hivDeaths, ccHealthState, hpvHealthState, newCC, totalPerAge, screenTreat, screenSympCCTreat, hivHealthState, newHiv, prepCov, newCirc);  
